...
public class AccessRuleApplet {
    ...
    public byte[] readRefreshTag() throws AccessControlException, IOException {
        ...
        if (responseApduSend.isStatus(EF.APDU_SUCCESS)) {
            try {
                BerTlv berTlvCreateDO = Response_DO_Factory.createDO(responseApduSend.getData());
                if (berTlvCreateDO instanceof Response_RefreshTag_DO) {
                    return ((Response_RefreshTag_DO) berTlvCreateDO).getRefreshTagArray();
                }
