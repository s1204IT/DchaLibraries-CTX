        ...
        for (int i = 0; i < childCount; i++) {
            ...
            if (tag instanceof AgendaByDayAdapter.ViewHolder) {
                ...
                if (viewHolder2.grayed) {
                    ...
                    if (!viewHolder2.allDay && viewHolder2.startTimeMilli <= jCurrentTimeMillis) {
                        return true;
                    }
            ...
            if (agendaItemByPosition != null) {
                if (selectedInstanceId != this.mWindowAdapter.getSelectedInstanceId() || !this.mShowEventDetailsWithAgenda) {
                    ...
                    if (tag instanceof AgendaAdapter.ViewHolder) {
                        j2 = ((AgendaAdapter.ViewHolder) tag).startTimeMilli;
                    } else {
                        j2 = j5;
                    }
                    if (agendaItemByPosition.allDay) {
                        long jConvertAlldayLocalToUTC = Utils.convertAlldayLocalToUTC(this.mTime, j5, this.mTimeZone);
                        long jConvertAlldayLocalToUTC2 = Utils.convertAlldayLocalToUTC(this.mTime, j6, this.mTimeZone);
                        j4 = jConvertAlldayLocalToUTC;
                        j3 = jConvertAlldayLocalToUTC2;
                    } else {
                        j3 = j6;
                        j4 = j5;
                    }
                    ...
                    calendarController.sendEventRelatedEventWithExtra(this, 2L, agendaItemByPosition.id, j4, j3, 0, 0, CalendarController.EventInfo.buildViewExtraLong(0, agendaItemByPosition.allDay), j2);
                }
            }
        }
    }
