...
class ExifParser {
    ...
    private static final LogUtil.Tag TAG = new LogUtil.Tag(ExifParser.class.getSimpleName());
    ...
    protected int next() throws ExifInvalidFormatException, IOException {
        ...
        if (readByteCount == i) {
            if (this.mIfdType == 0) {
                ...
                if (iIntValue < 4) {
                    LogHelper.w(TAG, "Invalid size of link to next IFD: " + iIntValue);
                } else {
                    long unsignedLong2 = readUnsignedLong();
                    if (unsignedLong2 != 0) {
                        LogHelper.w(TAG, "Invalid link to next IFD: " + unsignedLong2);
                    }
                }
            }
        }
            ...
            try {
                ...
                if (value instanceof IfdEvent) {
                    ...
                    if ((this.mNumOfTagInIfd * 12) + this.mIfdStartOffset + 2 > this.mApp1End) {
                        LogHelper.w(TAG, "Invalid size of IFD " + this.mIfdType);
                        return 5;
                    }
                    this.mNeedToParseOffsetsInCurrentIfd = needToParseOffsetsInCurrentIfd();
                    if (ifdEvent.mIsRequested) {
                        return 0;
                    }
                    ...
                    if (value instanceof ImageEvent) {
                        ...
                        return this.mImageEvent.mType;
                    }
                    ExifTagEvent exifTagEvent = (ExifTagEvent) value;
                    this.mTag = exifTagEvent.mTag;
                    if (this.mTag.getDataType() != 7) {
                        readFullTagValue(this.mTag);
                        checkOffsetOrImageTag(this.mTag);
                    }
                    if (exifTagEvent.mIsRequested) {
                        return 2;
                    }
                }
            } catch (IOException e) {
                LogHelper.w(TAG, "Failed to skip to data at: " + entryPollFirstEntry.getKey() + " for " + value.getClass().getName() + ", the file may be broken.");
            }
        }
        return 5;
    }
    ...
    protected int getStripIndex() {
        return this.mImageEvent.mStripIndex;
    }
    ...
    private ExifTag readTag() throws ExifInvalidFormatException, IOException {
        ...
        if (!ExifTag.isValidType(s2)) {
            LogHelper.w(TAG, String.format("Tag %04x: Invalid data type %d", Short.valueOf(s), Short.valueOf(s2)));
        ...
        if (dataType == 2 || dataType == 7 || dataType == 1) {
            ...
            if (this.mCorrespondingEvent.size() > 0 && this.mCorrespondingEvent.firstEntry().getKey().intValue() < this.mTiffStream.getReadByteCount() + componentCount) {
                ...
                if (value instanceof ImageEvent) {
                    ...
                    Map.Entry<Integer, Object> entryPollFirstEntry = this.mCorrespondingEvent.pollFirstEntry();
                    LogHelper.w(TAG, "Invalid thumbnail offset: " + entryPollFirstEntry.getKey());
                } else {
                    if (value instanceof IfdEvent) {
                        LogHelper.w(TAG, "Ifd " + ((IfdEvent) value).mIfd + " overlaps value for tag: \n" + exifTag.toString());
                    } else if (value instanceof ExifTagEvent) {
                        LogHelper.w(TAG, "Tag value for tag: \n" + ((ExifTagEvent) value).mTag.toString() + " overlaps value for tag: \n" + exifTag.toString());
                    }
