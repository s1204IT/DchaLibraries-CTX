    ...
    public SchemaGrammar parseSchema(XMLInputSource is, XSDDescription desc, Hashtable locationPairs) throws IOException {
        ...
        if (referType == 3) {
            ...
            if (grammar3 != null) {
                ...
                try {
                    if (grammar3.getDocumentLocations().contains(XMLEntityManager.expandSystemId(is.getSystemId(), is.getBaseSystemId(), false))) {
                        return grammar3;
                    }
        ...
        if (is instanceof DOMInputSource) {
            schemaRoot = getSchemaDocument(schemaNamespace, (DOMInputSource) is, referType == 3, referType, (Element) null);
        } else if (is instanceof SAXInputSource) {
            schemaRoot = getSchemaDocument(schemaNamespace, (SAXInputSource) is, referType == 3, referType, (Element) null);
        } else if (is instanceof StAXInputSource) {
            schemaRoot = getSchemaDocument(schemaNamespace, (StAXInputSource) is, referType == 3, referType, (Element) null);
        } else if (is instanceof XSInputSource) {
            schemaRoot = getSchemaDocument((XSInputSource) is, desc);
        } else {
            schemaRoot = getSchemaDocument(schemaNamespace, is, referType == 3, referType, (Element) null);
        }
        if (schemaRoot == null) {
                ...
                if (isDocument) {
                    try {
                        ...
                        if (t instanceof IOException) {
                            IOException exception = (IOException) t;
                            e = exception;
                            return getSchemaDocument1(mustResolve, true, schemaSource, referElement, e);
                        }
