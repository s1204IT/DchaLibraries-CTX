...
public class NetworkMonitor extends StateMachine {
    ...
    protected CaptivePortalProbeResult isCaptivePortal() throws Throwable {
        ...
        sendNetworkConditionsBroadcast(true, captivePortalProbeResultSendDnsAndHttpProbes.isPortal(), jElapsedRealtime, SystemClock.elapsedRealtime());
    ...
    private void sendNetworkConditionsBroadcast(boolean z, boolean z2, long j, long j2) {
        ...
        Intent intent = new Intent(ConnectivityConstants.ACTION_NETWORK_CONDITIONS_MEASURED);
        switch (this.mNetworkAgentInfo.networkInfo.getType()) {
            case 0:
                intent.putExtra(ConnectivityConstants.EXTRA_NETWORK_TYPE, this.mTelephonyManager.getNetworkType());
                ...
                for (CellInfo cellInfo : allCellInfo) {
                    if (cellInfo.isRegistered()) {
                        ...
                        if (cellInfo instanceof CellInfoCdma) {
                            intent.putExtra(ConnectivityConstants.EXTRA_CELL_ID, ((CellInfoCdma) cellInfo).getCellIdentity());
                        } else if (cellInfo instanceof CellInfoGsm) {
                            intent.putExtra(ConnectivityConstants.EXTRA_CELL_ID, ((CellInfoGsm) cellInfo).getCellIdentity());
                        } else if (cellInfo instanceof CellInfoLte) {
                            intent.putExtra(ConnectivityConstants.EXTRA_CELL_ID, ((CellInfoLte) cellInfo).getCellIdentity());
                        } else if (cellInfo instanceof CellInfoWcdma) {
                            intent.putExtra(ConnectivityConstants.EXTRA_CELL_ID, ((CellInfoWcdma) cellInfo).getCellIdentity());
                        } else {
            ...
            case 1:
                ...
                if (connectionInfo != null) {
                    intent.putExtra(ConnectivityConstants.EXTRA_SSID, connectionInfo.getSSID());
                    intent.putExtra(ConnectivityConstants.EXTRA_BSSID, connectionInfo.getBSSID());
                } else {
        ...
        intent.putExtra(ConnectivityConstants.EXTRA_CONNECTIVITY_TYPE, this.mNetworkAgentInfo.networkInfo.getType());
        intent.putExtra(ConnectivityConstants.EXTRA_RESPONSE_RECEIVED, z);
        intent.putExtra(ConnectivityConstants.EXTRA_REQUEST_TIMESTAMP_MS, j);
        if (z) {
            intent.putExtra(ConnectivityConstants.EXTRA_IS_CAPTIVE_PORTAL, z2);
            intent.putExtra(ConnectivityConstants.EXTRA_RESPONSE_TIMESTAMP_MS, j2);
        }
        this.mContext.sendBroadcastAsUser(intent, UserHandle.CURRENT, ConnectivityConstants.PERMISSION_ACCESS_NETWORK_CONDITIONS);
    }
