...
class UserSwitchingDialog extends AlertDialog implements ViewTreeObserver.OnWindowShownListener {
    ...
    void inflateContent() {
        ...
        if (UserManager.isSplitSystemUser() && this.mNewUser.id == 0) {
            string = resources.getString(R.string.mmcc_authentication_reject_msim_template, this.mOldUser.name);
            ...
            if (str == null) {
                string = resources.getString(R.string.mmcc_illegal_ms, this.mNewUser.name);
            } else {
                string = str;
            }
        }
