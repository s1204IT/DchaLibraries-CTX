...
class RemoteBugreportUtils {
    ...
    static Notification buildNotification(Context context, int i) {
        ...
        if (i == 2) {
            color.setContentTitle(context.getString(R.string.managed_profile_label)).setProgress(0, 0, true);
        } else if (i == 1) {
            color.setContentTitle(context.getString(R.string.mediasize_na_ledger)).setProgress(0, 0, true);
        } else if (i == 3) {
            color.addAction(new Notification.Action.Builder((Icon) null, context.getString(R.string.battery_saver_charged_notification_summary), PendingIntent.getBroadcast(context, NOTIFICATION_ID, new Intent("com.android.server.action.REMOTE_BUGREPORT_SHARING_DECLINED"), 268435456)).build()).addAction(new Notification.Action.Builder((Icon) null, context.getString(R.string.low_internal_storage_view_text), PendingIntent.getBroadcast(context, NOTIFICATION_ID, new Intent("com.android.server.action.REMOTE_BUGREPORT_SHARING_ACCEPTED"), 268435456)).build()).setContentTitle(context.getString(R.string.low_internal_storage_view_title)).setContentText(context.getString(R.string.low_internal_storage_view_text_no_boot)).setStyle(new Notification.BigTextStyle().bigText(context.getString(R.string.low_internal_storage_view_text_no_boot)));
        }
        return color.build();
    }
}
